...
A simple CNN model for MNIST using PyTorch framework in a Docker container.
...
```sh
docker run -t --rm -v volume-mnist:/app/state pytorch-mnist --mode fit
```
...
```sh
docker run -t --rm -v volume-mnist:/app/state pytorch-mnist --mode predict
```
...
```sh
docker run -t --rm -v volume-mnist:/app/state pytorch-mnist --mode eval
```
